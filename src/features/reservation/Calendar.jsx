...
const Calendar = ({ disabledDates, reservedDates, selectedDate, onDateSelect }) => {
  ...
  const getDateClass = (date) => {
    const base =
      'w-8 h-8 flex items-center justify-center text-xs font-medium border-none rounded-full cursor-pointer mx-auto transition-all duration-150 ';
    ...
    if (isToday(date)) {
      return base + 'border-2 border-[#F59E0B] text-[#333] font-bold bg-transparent hover:bg-[#DBEAFE]';
    }
  ...
  return (
    <div>
      <div className="flex items-center gap-1.5 text-[13px] font-bold text-[#111] mb-3.5">
      ...
      <div className="flex items-center justify-center gap-4 mb-3">
        <button
          ...
          className="bg-transparent border-none cursor-pointer px-2 py-1 text-[#555] text-base rounded hover:bg-[#f0f0f0] transition-colors"
        >
          ‹
        </button>
        <div className="text-sm font-bold text-[#111] min-w-[80px] text-center">
      ...
      <div className="grid grid-cols-7 gap-0.5">
        {WEEK_DAYS.map((day) => (
          <div
            key={day.label}
            className={`text-center text-[11px] font-semibold pb-2 ${day.color}`}
          >
            {day.label}
          </div>
        ))}
      ...
      <div className="flex items-center gap-3.5 mt-3 text-[11px] text-[#555]">
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded-full bg-[#6B7280]" />
          <span>예약불가</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded-full border-2 border-[#F59E0B] bg-white" />
          <span>오늘</span>
        </div>
        <div className="flex items-center gap-1.5">
